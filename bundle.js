...
(function displayGDP() {
	...
	var canvas = d3.select('.gdp').append("svg").attr("class", "svg").attr("width", width + padding * 2).attr("height", height + padding * 2).attr("border", "1px solid black").append("g").attr("transform", "translate(" + padding + "," + padding + ")");
	...
	d3.json("https://raw.githubusercontent.com/FreeCodeCamp/ProjectReferenceData/master/GDP-data.json", function (err, response) {
		if (err) {
			...
			yScale.domain([0, d3.max(response.data, function (d) {
				return d[1];
			})]);
			...
			canvas.append("g").attr("transform", "translate(0, " + 700 + ")").call(xAxis).selectAll("text").attr("class", "x-text").attr("dy", ".8em").attr("dx", "-1.2em").attr("transform", "rotate(-45)");

			canvas.append("g").call(yAxis).append("text").attr("class", "y-text").attr("y", 40).attr("x", -200).attr("dy", "1em").attr("transform", "rotate(-90)").attr("text-anchor", "end").text("Gross Domestic Product (in billions)");

			canvas.selectAll("rect").data(response.data).enter().append("rect").attr("width", 5).attr("height", function (d) {
				...
				//return yScale(d[1]);
				return yScale(d[1]);
			}).attr("stroke", "black").on("mouseover", function () {
				...
				return tooltip.style("top", event.pageY - 110 + "px").style("left", event.pageX + 10 + "px").html("<h3>$ " + d[1] + " Billion</h3><br><p>" + date.getFullYear() + " - " + months[date.getMonth()] + "" + "</p>");
			});

			var tooltip = d3.select("body").append("div").style("position", "absolute").style("z-index", 10).style("visibility", "hidden").style("background", "rgba(255,255,255,.9)").style("width", "150px").style("height", "80px").style("border-radius", "5px").style("border", "2px solid black").style("padding-left", "10px");
		}
	});
})();
